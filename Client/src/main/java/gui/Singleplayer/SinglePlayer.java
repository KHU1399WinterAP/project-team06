...
public class SinglePlayer extends JFrame {
    ...
    private void initRecords() {
        ...
        client.sendRequest("INIT_RECORDS");

        for (JButton jButton:Arrays.asList(bestRecordEnglishButton,bestRecordMathButton,
                bestRecordFoodButton,bestRecordScienceButton,bestRecordCommonButton,
    ...
    private void recordSelected(String column, String category) {
        ...
        client.sendRequest("USER_ORDER_BY_RECORDS");
        client.sendRequest(column);



        new ScoreBoard(this,fillingArray(), category);
    }

    private ArrayList<ScoreBoardInformation> fillingArray(){
        ArrayList<ScoreBoardInformation> records=new ArrayList<>();
        int recordsSize=client.getResponseInt();

        for (int i=0;i<recordsSize;i++){
            ScoreBoardInformation score=new ScoreBoardInformation(client.getResponse(),
