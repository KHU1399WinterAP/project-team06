...
public class ProfileSettings extends JFrame {
    ...
    private void selectAPhoto() {
        for (int i = 0; i < facesButton.size(); i++) {
            facesButton.get(i).setIcon(new ImageIcon(Objects.requireNonNull(getClass().getResource(
                    "/main/resources/icons/Avatars/face" + (i + 1) + ".png"))));
        }
    }
    ...
    private void applyChangesButtonActionPerformed(ActionEvent e) {
        ...
        client.sendRequest("UPDATE_SETTINGS");
        client.sendRequest(activeUser.username);
        client.sendRequest(Dashboard.password);
        client.sendRequest(String.valueOf(activeUser.profilePicture));

        client.sendRequest(newUsername);
        client.sendRequest(newPassword);
        client.sendRequest(String.valueOf(profilePicture));
        ...
        if (response.equals("REJECT")) {
            ...
            if (response.equals("ALREADY_EXIST"))
                error();
            else if (response.equals("ACCEPT")) {

                String changedElement = client.getResponse();

                while (!changedElement.equals("OVER")) {

                    if (changedElement.equals("CHANGE_PROFILE_PICTURE"))
                        activeUser.profilePicture = profilePicture;

                    if (changedElement.equals("CHANGE_USERNAME"))
                        activeUser.username = newUsername;

                    if (changedElement.equals("CHANGE_PASSWORD")) {
                        activeUser.password = Objects.hash(newPassword);
                        Dashboard.password = newPassword;
                    }

                    changedElement = client.getResponse();
                }
                applyChanges();
            } else
                error();
